...
public class CardController : MonoBehaviour
{
    ...
    public void DrawCurrentCards(Character character)
    {
        ...
        {
            hand[i].SetPosition(transform.TransformPoint(new Vector3(start.x + xSeperation * i, start.y + ySeperation * i)));
        }
        if (cards.Count % 2 == 1)
        {
            int index = cards.Count / 2;
            hand[index].SetPosition(new Vector3(center.x, center.y));
        }
        for (int i = cards.Count / 2 + 1; i < cards.Count;  ++i) {
            hand[i].SetPosition(new Vector3(start.x + xSeperation * i, start.y + ySeperation * -(i - cards.Count)));
        }
